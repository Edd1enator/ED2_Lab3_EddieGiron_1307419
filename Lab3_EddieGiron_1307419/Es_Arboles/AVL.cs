...
namespace Es_Arboles
{
    public class AVL <T> : NoLinear<T> where T : IComparable<T>
    {
        ...
        public List<T> DevuelveBusqueda(Node<T> nodo, Delegate delegado)
        {
            List<T> ret = new List<T>();
            Node<T> aux = Raiz;
            ObtenerBusqueda(aux, nodo, delegado, ret);
            return ret;
        }
        public List<T> ObtenerBusqueda(Node<T> inicio, Node<T> Buscando, Delegate delegado, List<T> Lista)
        {
            if (Buscando.Valor != null)
            {
                if (Convert.ToInt32(delegado.DynamicInvoke(Buscando.Valor, inicio.Valor)) == 0)
                {
                    Lista.Add(inicio.Valor);
                    if (inicio.Izquierdo.Valor != null)
                    {
                        if (Convert.ToInt32(delegado.DynamicInvoke(Buscando.Valor, inicio.Izquierdo.Valor)) == 0)
                        {
                            return ObtenerBusqueda(inicio.Izquierdo, Buscando, delegado, Lista);
                        }
                    }
                    if (inicio.Derecho.Valor != null)
                    {
                        if (Convert.ToInt32(delegado.DynamicInvoke(Buscando.Valor, inicio.Derecho.Valor)) == 0)
                        {
